...
int queryInfo(struct task *send_task, struct proTime *pro_time)
{
	...
	snprintf(sql_buf, sizeof(sql_buf), "select B.programId,B.playTime from Program_Info A,Program_Time B where A.RadioID = %d and B.ProgramID = A.ProgramID and B.Day=CURDATE() order by playTime", send_task->radioId);
